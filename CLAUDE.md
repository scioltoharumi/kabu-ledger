...
- 割安さの単一軸ソートを実装しない
- **チャート形状（6か月・楽天の9分類）だけは画像判定。** `src/shape_chart.py` が描いた
  線画を Claude が見て分類する（2026-09-05。「判定はコード」の唯一の例外）。
  ゲートに使わず「画像判定※」と明示し、画像の SHA-256 と履歴を残す。
  週足MAの傾きは参考行（旧「見送(トレンド)」は廃止）。非保有×雲の下は「見送(雲の下)」
...
- `watch` は `active` / `excluded` の固定語彙（未記載＝`active`）。
  **語彙外は `active` に倒す**（打ち間違いで銘柄が黙って台帳から消える方が悪い）。
  倒した結果に気づけるよう `checks` が語彙外を FAIL にする。
  `excluded` は取得・判定・週次追記から外れるが**消さない**。
  一覧と銘柄ページに「対象外」と理由を必ず出す（消す方法は用意しない）。
  `watch_reason` は必須。再登場時は intake が人間に判断を仰ぐ（自動で戻さない）
...
- `estimates/{code}.yaml` 次期売上・利益のフェルミ推定（追記型・過去版は消さない。
  値の選定は basis 明示で Claude 起案。承認工程は置かず、調べた結果をそのまま載せる
  （2026-09-05）。計算・感度は `src/estimate.py`）
