...
- `master.yaml` の既存銘柄を**消さない・書き換えない**。追加だけする
  （唯一の例外が上の「対象外の再登場」で、**人間の指示があったときだけ** `watch` を戻す）
...
**★ `watch: excluded` の銘柄が再び出てきたら、そこで止まって人間に聞く。**

一度「対象外」にした銘柄が、スクリーニングやセグメント分析で再登場することがある。
このとき **勝手に `active` に戻さない**し、**黙って読み飛ばしもしない**。どちらも
判断を人間から奪う。出すのはこの形:
...
再登場した対象外の銘柄:
  3851 日本一ソフトウェア
    外した日   : 2026-08-28
    外した理由 : （master.yaml の watch_reason をそのまま引く）
    今回の再登場: （どのスクリーニング／どのセグメントで出てきたか）
  → 監視に戻しますか（watch: active）／対象外のままにしますか
```

戻すと決まったら `watch` を `active` にし、`watch_reason` は**消さずに残す**
（`watch_since` は戻した日に更新する）。外していた期間が記録から消えると、
「なぜ履歴に穴があるのか」が後から分からなくなる。
